...
exports.update = async (id, data, result) => {
    try {
        const query = `UPDATE ${tableName} SET parent_id = ?,name =?, role_id = ?,email=?,account=?,phone=?,address=?,web_page=?,active =?,updated_at=? WHERE id =?`;
        db.query(
            query,
            [
                data.parent_id,
...
//uploadImage
exports.uploadImage = async (data, result) => {
    try {
        const query = `UPDATE ${tableName} SET avatar =?, thumb = ? WHERE id = ?`;
        db.query(query, [data.avatar, data.thumb, data.id], (err, dataRes) => {
            if (err) {
                return result({ msg: constantNotify.ERROR }, null);
            }
            if (dataRes.affectedRows === 0) {
